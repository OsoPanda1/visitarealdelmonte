...
type FederationId =
  "gubernamental" | "cultural" | "economica" | "tecnologica" | "educativa" | "salud";
...
const getFacetTone = (poi: TerritoryPOI) => {
  const federationId =
    (poi as unknown as { federationId?: FederationId }).federationId ?? "gubernamental";
...
export default function TerritorialSVGMap({
  ...
  return (
    <div className="glass-card shadow-premium relative w-full overflow-hidden rounded-3xl border border-[hsl(var(--gold)/0.2)]">
      <div className="aurora-bg pointer-events-none absolute inset-0 opacity-70" aria-hidden />
      ...
      {active && (
        ...
        >
          ...
          <p className="mt-2 text-sm text-[hsl(var(--foreground)/0.85)]">{active.description}</p>
