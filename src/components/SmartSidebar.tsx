...
const SECTIONS: SidebarSection[] = [
  ...
  },
  {
    ...
    color: "#f43f5e",
    badge: "DONAR",
    links: [{ to: "/donar", label: "Donar ahora", icon: HandCoins }],
  },
];
...
function isActive(pathname: string, to: string): boolean {
  return (
    Boolean(matchPath({ path: to, end: to === "/" || !to.includes("/:") }, pathname)) ||
                          ...
                          <div className="ml-4 mt-0.5 space-y-0.5 border-l border-white/5 pl-2">
                            {section.links.map((link) => {
                              ...
                              return (
                                ...
                                >
                                  {LinkIcon && (
                                    <LinkIcon className="h-3 w-3 shrink-0 opacity-60 group-hover:opacity-100" />
                                  )}
