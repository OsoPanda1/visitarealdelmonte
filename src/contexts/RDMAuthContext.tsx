  ...
  const loadProfileAndRoles = useCallback(async (uid: string) => {
    try {
      ...
      if (profileError || rolesError) {
        const messages = [profileError?.message, rolesError?.message].filter(Boolean) as string[];
        if (messages.length > 0) {
          setError(`[auth] Error al cargar perfil/roles: ${messages.join(" | ")}`);
        }
      } else {
        setError(null);
      }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        if (sessionError) {
          setError(`[auth] Error al recuperar sesión inicial: ${sessionError.message}`);
          setSession(null);
          setUser(null);
        } else {
