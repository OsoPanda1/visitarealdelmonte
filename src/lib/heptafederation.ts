...
import { federationHealth, federationModuleStatus } from "@/core/metrics/prometheus";
 ...
 */
export function applyDecisionToHeptafederation(decision?: IsabellaDecision): FederationModule[] {
  ...
  currentFederation = currentFederation.map((module) => {
    ...
    const newStatus: FederationStatus =
      operationalScore < 0.7 ? "DEGRADED" : operationalScore < 0.85 ? "IDLE" : module.status;
...
export function getFederationStats(): {
  ...
  return {
    ...
    avgOperationalScore:
      currentFederation.reduce((acc, m) => acc + m.operationalScore, 0) / currentFederation.length,
    criticalAlerts: criticalModules.map(
      (m) => `${m.name} en estado critico (${(m.operationalScore * 100).toFixed(1)}%)`,
    ),
  };
}
...
const LOCAL_FEDERATION_DEFAULTS: LocalFederationStatus[] = [
  ...
  {
    id: Federacion.MINERIA_PLATERIA,
    name: "Plateria y Mineria",
    activeBusiness: 8,
    satisfaction: 0.85,
    revenue24h: 23000,
    trending: "stable",
  ...
  {
    id: Federacion.GOBIERNO_DIGITAL,
